export default function How(props) {
  return (
    <>
      <section className="section light large flex-direction-override">
        <h1>
          <em>How</em> to use your deficit
        </h1>
        <div className="bullet-container how">
          <div>
            <span className="number">1</span>
            Calculate how many calories you need to consume over a week to lose
            weight
          </div>
          <div>
            <span className="number">2</span>
            Choose which days you might consume more calories, like the weekend
          </div>
          <div>
            <span className="number">3</span>
            Plan how many calories you can consume each day to be in a total
            calorie deficit by the end of the week
          </div>
          <div>
            <span className="number">4</span>
            Track your calories and use our weekly approach to eat flexibly,
            socialise and still lose weight
          </div>
        </div>
      </section>
    </>
  );
}
